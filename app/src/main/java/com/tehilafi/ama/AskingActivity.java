...
public class AskingActivity extends Activity {
    ...
    private int score, numImportantQuestions, num_question, counter_checkBox = 0;
...
//************************************  Get  data from Users DB  *************************************
        DatabaseReference reffU = FirebaseDatabase.getInstance().getReference( "Users" );
        Query myQuery = reffU.orderByChild("token");
        myQuery.addChildEventListener( new ChildEventListener() {
           ...
           public void onChildAdded(@NonNull DataSnapshot snapshot, @Nullable String previousChildName) {
               ...
               if (distanceInMeters < 300) {
                   if(token != myToken && String.valueOf(idd) != iduser)
                       tokens.add( token + "," + idd);
               }
           }
        ...
        send.setOnClickListener( new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!edtContent.getText().toString().equals( "" )) {
                    ...
                    sendNotification( tokens, "שאלה חדשה",  edtContent.getText().toString().trim(), "question", "MyAnswerActivity" );
