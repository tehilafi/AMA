...
public class UploadPhotoAndVideos {
    ...
    public static boolean uploadImageFromCamera(Context mContext, final byte[] bb, final String numAns, final String iduser, final String from ) {

        storage = FirebaseStorage.getInstance();
        storageReff = storage.getReference();
        StorageReference reffC = null;

        if (from == "pic")
            reffC = storageReff.child( "picture answer/" + numAns );
        else if (from == "profil")
            reffC = storageReff.child( "profile picture/" + iduser);
